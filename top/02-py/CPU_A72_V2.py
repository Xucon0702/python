...
# all_process_list = []
list_process_list = ['perf','svcamera']
# list_process_list2 = []
process_mode = False
...
def trans_csv(in_file,out_file):
    ...
    #     str_list = line.split()
    #     if len(str_list) > 11 and str_list[1] == 'root':
    #         if str_list[11] not in all_process_list:
    #             all_process_list.append(str_list[11])
    #             print('all_process_list: ' + str_list[11])
    # in_fileObj.seek(0)
    #
    # for y in all_process_list:
    #     if y in list_process_list:
    #         list_process_list2.append(y)
    #         print('list_process_list2 ' + y)

    for line in in_fileObj.readlines():
        data_list = line.split()  # split string with blank char
        if process_mode == True:
            if len(data_list) == 12 and data_list[1] == 'root':
                line_continue = False
                for x in list_process_list:
                    if x in data_list[11]:
                        line_continue = True
                if line_continue == False:
                    continue
        # pass the blank line
        if len(data_list) < 3:
            continue
        ...
        if data_list[0].find("top") != -1 or data_list[0].find("Tasks:") !=-1 or data_list[0].find("%Cpu(s)") !=-1 \
            ...
            if data_list[0] == '%Cpu0' or data_list[0] == '%Cpu1':
                ...
                if data_list[8] != '%CPU':
                    data_list[8] = str(float(data_list[8]))
...
def plot(csv_file):
    ...
    '''
    # load mtop result
    df = pd.read_csv(csv_file, sep='\\s+', parse_dates=['TS'], names=["TS", "PID", "USER", "PR","NI", "VIRT", "RES", "SHR", "S", "%CPU", "%MEM", "TIME+", "COMMAND"])
...
if __name__ == "__main__":
    print("please input:python ./CPU_A72.py ProcessName1 ProcessName2 ProcessName3 ... ProcessName10")
    args=sys.argv
    if len(args) > 1:
        process_mode = True
        for k in args:
            list_process_list.append(k)
            print(k)


    dir_path = list_dir_in_folder('./')
    for k in dir_path:
        for j in list_files_in_folder(k):
            if 'top.txt' in j:
                trans_csv(j, str(j).split('.txt')[0]+'.out')
                plot(str(j).split('.txt')[0]+'.out')
